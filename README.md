...
- **Inline mode** (default) — condenses each paragraph in place, keeping the original page layout
...
- **Auto-condense** — optionally runs on every page load
- **Lifetime score** — tracks cumulative words cut, mapped to book equivalents
...
- **Auto-run sends every page you visit.** A warning is shown when you enable this. The extension requests additional permissions only when auto-run is turned on.
...
- **API costs are on you.** Each condensation makes one API call. There is no rate limiting.
